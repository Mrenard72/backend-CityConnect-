...
// Envoyer un message – version avec findByIdAndUpdate
router.post('/:conversationId/message', authMiddleware, async (req, res) => {
  try {
    ...
    // Utilisation de l'opérateur $push pour ajouter le message et $set pour mettre à jour lastUpdated
    const update = {
    ...
    if (!updatedConversation || !updatedConversation.messages || !updatedConversation.messages.length) {
